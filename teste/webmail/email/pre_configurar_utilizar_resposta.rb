...
seed_file = File.new("pre_configurar_utilizar_resposta_seed.rb", "w")
...
begin
  ...
  seed_file.puts "ComoPreconfigurarUtilizarRespostaTutorial.create(title: '#{title}', date: '#{date}')"

  text_elements.each do |text|
    content = text.text.strip
    seed_file.puts "ComoPreconfigurarUtilizarRespostaTutorialContent.create(tutorial_id: ComoPreconfigurarUtilizarRespostaTutorial.last.id, content: '#{content}')"
  end
  ...
  image_urls.each_with_index do |url, index|
    seed_file.puts "Image.create(tutorial_id: ComoPreconfigurarUtilizarRespostaTutorial.last.id, url: '#{url}', caption: 'Imagem #{index + 1}')"
  end
