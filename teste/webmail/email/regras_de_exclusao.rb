...
seed_file = File.new("regras_de_exclusao_seed.rb", "w") # Append to existing seed file

begin
  doc = Nokogiri::HTML(URI.open('https://www.agenciaw3.digital/ajuda/email-corporativo/email/tutorial/regras-de-exclusao'))
  ...
  text_elements.each do |text|
    ...
    seed_file.puts "CriarFiltrosTutorialContent.create(tutorial: CriarFiltrosTutorial.last, content: '#{content}')"
  end
  ...
  image_urls.each_with_index do |url, index|
    seed_file.puts "Image.create(tutorial: CriarFiltrosTutorial.last, url: '#{url}', caption: 'Imagem #{index + 1}')"
  end
