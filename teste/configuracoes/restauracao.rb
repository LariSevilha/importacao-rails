...
seed_file = File.new("restoration_seed.rb", "w")

begin
  doc = Nokogiri::HTML(URI.open('https://agenciaw3.digital/ajuda/backup/tutoriais-para-backup/tutorial/restauracao-de-arquivos'))

  title = doc.css('div.cb-text.medium-editor-element p b').text.strip
  text_elements = doc.css('div.cb-text.medium-editor-element p')
  image_links = doc.css('div.cb-figure img')

  seed_file.puts "Tutorial.create(title: '#{title}')"
